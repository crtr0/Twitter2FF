...
class FriendFeed(object):
    ...
    def fetch_user_profiles(self, nicknames, **kwargs):
        """Returns user profiles for the given nicknames.

        Authentication is required for private users.
        """
        #return self._fetch(
        #    "/api/profiles", nicknames)
        return self._fetch_feed("/api/profiles", nickname=",".join(nicknames), **kwargs)
    ...
    def _fetch(self, uri, post_args, **url_args):
        ...
        APIKEY = getattr(settings, "APIKEY", "twitter2ff-0581bdf38d")
        if APIKEY:
